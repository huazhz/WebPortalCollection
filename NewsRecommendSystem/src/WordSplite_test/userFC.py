...
def recommendByUserFC(userid,user_to_rate,key_to_user,user_to_key,user_to_news,all_news):   
    ...
    return recommend_list_byUser,recommend_list_byTag

def Get_userInfo():
    fr = open(Global_param.test_root+'test/user_keys.txt')
    ...
    for line in fr.readlines():
        ...
        for i in range(0,length,2):
            key = info[i]
            num = info[i+1]
            user_keys[user].append(key)
            user_KeytoRate[user].append([key,num])
            if key not in key_users.keys():
                key_users[key] = []
                key_users[key].append(user)
            else:
                key_users[key].append(user)
    ...
    for line in fr1.readlines():
        ...
        if user not in user_news.keys():
            user_news[user] = []
            user_news[user].append(news)
        else:
            if news not in user_news[user]:
                user_news[user].append(news)    
                
    return user_KeytoRate,user_keys,key_users,user_news

def Get_newsTag():
    fr = open(Global_param.test_root+'test/newstag.txt')
    news_to_tags = {}
    
    for line in fr.readlines():
        news = line.strip().split('\t')[0]
        tags = line.strip().split('\t')[1:]
        #新闻标题去重
        if news not in news_to_tags:
            news_to_tags[news] = tags
    return news_to_tags
    


user_KeytoRate,user_keys,key_users,user_to_news = Get_userInfo()
userID = "5218791"
news_to_tags = Get_newsTag()
